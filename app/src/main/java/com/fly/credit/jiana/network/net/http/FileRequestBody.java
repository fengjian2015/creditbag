...
/**
 * lucksoft
 * Created by AndroidDevelop on 2020/6/20.
 * 158045632@qq.com
 */
...
public class FileRequestBody extends RequestBody {
    ...
    private final class ByteSink extends ForwardingSink {
        ...
        public void write(Buffer source, long byteCount) throws IOException {
            ...
            runOnUiThread(mByteLength, contentLength());
        }
    }
    ...
    private void runOnUiThread(final long currentLength, final long contentLength){
        NetClient.getInstance().getDelivery().post(new Runnable() {
            @Override
            public void run() {
                if (mLoadingListener != null) {
                    mLoadingListener.onProgress(currentLength, contentLength);
                }
            }
        });
    }
